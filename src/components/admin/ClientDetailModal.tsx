...
const ClientDetailModal = ({ open, onClose, userId, userName, avatarUrl }: Props) => {
  ...
  const [updatingRoulette, setUpdatingRoulette] = useState(false);
  ...
  const handleToggleRoulette = async (enabled: boolean) => {
    setUpdatingRoulette(true);
    const { error } = await supabase
      .from("profiles")
      .update({ allow_welcome_roulette: enabled } as any)
      .eq("user_id", userId);
    
    if (error) {
      toast.error("Erro ao atualizar roleta");
    } else {
      toast.success(enabled ? "Roleta permitida para este cliente" : "Roleta bloqueada para este cliente");
      await loadData();
    }
    setUpdatingRoulette(false);
  };
                    ...
                    <TabsContent value="dados" className="mt-0">
                      {editing ? (
                        <div className="space-y-1">
                          ...
                          <EditableRow icon={Phone} label="Telefone" value={editData.phone} onChange={(v) => {
                            const digits = v.replace(/\D/g, "").slice(0, 11);
                            let masked = digits;
                            if (digits.length > 2) masked = `(${digits.slice(0, 2)}) ${digits.slice(2)}`;
                            if (digits.length > 7) masked = `(${digits.slice(0, 2)}) ${digits.slice(2, 7)}-${digits.slice(7)}`;
                            setEditData(d => ({ ...d, phone: masked }));
                        ...
                        <div className="space-y-1">
                          ...
                          <InfoRow icon={Calendar} label="Cadastro" value={profile?.created_at ? new Date(profile.created_at).toLocaleDateString("pt-BR") : null} />
                          <InfoRow icon={Clock} label="Último acesso" value={profile?.last_seen ? new Date(profile.last_seen).toLocaleString("pt-BR") : "Nunca acessou"} />
                          <div className="flex items-center justify-between py-3 px-1 border-t border-border/50 mt-4 bg-primary/5 rounded-xl">
                            <div className="flex items-center gap-2.5">
                              <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center">
                                <Ticket className="w-4 h-4 text-primary" />
                              </div>
